...
def make_defaults():
    ...
    font_path = (
        "/Users/lucas/Library/Fonts/FiraSans-Regular.ttf"  # Your font path goes here
    )
...
def wandb2df(df_start, step_column):
    ...
    df = df.pivot_table(
        index=[step_column, "id"], columns="var", values="value"
    ).reset_index()
...
def get_track_and_states_df(df_summary, df_track_initial, df_states_initial, task=None):
    ...
    df_states = df_states.apply(
        lambda x: np.rad2deg(x) if x.name not in ["step", "V", "he"] else x
    )
    df_states["step"] *= 0.01
...
def plot_states(df_track, df_states, with_legend=True):
    ...
    def plot_data(data, x, y, ax, ylabel, ref_data=None, ref_y=None):
        ...
        if ref_data is not None and ref_y is not None:
            sns.lineplot(
                data=ref_data, x=x, y=ref_y, ax=ax, color=ref_color, linestyle="dashed"
            )
    ...
    plot_data(
        df_track, "step", "theta", axes[2, 0], r"$\theta$ [deg]", df_track, "theta_ref"
    )
    plot_data(df_track, "step", "phi", axes[2, 1], r"$\phi$ [deg]", df_track, "phi_ref")
    plot_data(df_states, "step", "V", axes[3, 0], r"$V$ [m/s]")
    plot_data(
        df_track, "step", "beta", axes[3, 1], r"$\beta$ [deg]", df_track, "beta_ref"
    )
    ...
    if with_legend:
        ...
        ref_line = mlines.Line2D(
            [], [], color=ref_color, linestyle="dashed", label="Reference"
        )
        fig.legend(
