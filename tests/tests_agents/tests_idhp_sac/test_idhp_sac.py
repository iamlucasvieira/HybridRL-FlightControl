...
class TestIDHPSAC:
    ...
    #     env = env()
    #     agent = IDHPSAC(
    #         env, sac_kwargs=dict(learning_starts=1, buffer_size=1, batch_size=1, device="cpu")
    #     )
    #
    ...
    def test_envs_are_idhp_like(self, env):
        ...
        if isinstance(env, LTIEnv):
            assert agent.sac.env.get_obs == get_observation("states + ref + error")
            assert agent.idhp.env.get_obs == get_observation("states + error")
        else:
            assert agent.sac.env.get_obs == get_observation("states + ref + error")
            assert agent.idhp.env.get_obs == get_observation("idhp_citation")
